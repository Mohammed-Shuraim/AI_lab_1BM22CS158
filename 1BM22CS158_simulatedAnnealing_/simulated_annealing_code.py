...
# Simulated annealing function with detailed logging and tracking
def simulated_annealing_with_tracking(initial_state, schedule, max_iterations=1000):
    current_state = initial_state
    current_cost = calculate_cost(current_state)
    costs = []         # List to track the cost at each iteration
    temperatures = []  # List to track temperature values for each iteration
    ...
    for t in range(max_iterations):
        ...
        # Append the current cost and temperature to the lists for plotting
        costs.append(current_cost)
        temperatures.append(T)

        # If the state is accepted, update current state and cost
        if accept:
            current_state, current_cost = next_state, next_cost

    # Append the final cost and temperature
    costs.append(current_cost)
    temperatures.append(T)
    return costs, temperatures

# Linear schedule function to decrease temperature over iterations
...
# Get the number of queens and the initial state
try:
...
# Run the simulated annealing process and collect cost values and temperature values for each iteration
costs, temperatures = simulated_annealing_with_tracking(initial_state, linear_schedule)

# Plotting the cost across iterations
plt.figure(figsize=(14, 6))

# Subplot 1: Cost vs. Iterations
plt.subplot(1, 2, 1)
