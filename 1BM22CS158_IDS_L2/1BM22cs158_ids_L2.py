...
def iterative_deepening_search(root, goal):
    ...
    while True:
        ...
        if result == "Goal Found":
            ...
            return "Goal Found"
        if result == "Cutoff":
            print(f"Cutoff occurred at depth {depth}, increasing depth\n")
            depth += 1
        else:
            print(f"Failure at depth {depth}, stopping search")
            return "Failure"

def depth_limited_search(node, goal, limit):
    print(f"Visiting Node: {node.value}, Depth Limit: {limit}")
...
# Run iterative deepening search and print the output
